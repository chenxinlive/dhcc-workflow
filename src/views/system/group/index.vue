            ...
            <el-row>
              <el-col :span="24">
                <template v-if="isAdd==false">
                  ...
                  <el-table :data="list" stripe style="width: 100%;margin-top: 10px" ref="groupUser">
...
<script>
import { getGroupList, getUserByGorupId, addGroup, updataGroup, deleteGroup , addUserGroup, deleteUserGroup} from '@/api/orgm/group'
...
export default {
  ...
  methods:{
    ...
    saveAssigneeByUser() {
      ...
      addUserGroup({userlist:this.selectData,userDatalist:this.list,groupid:this.groupParams.id}).then(res => {
        if(res.code=="200"){
          this.$message.success("添加成功");
          this.getGroupUser();
        }
    ...
    deleteGroupUsers(row){
      deleteUserGroup(row.username,this.groupParams.id).then(res => {
        if(res.code=="200"){
          this.$message.success("删除成功");
          this.getGroupUser();
        }
