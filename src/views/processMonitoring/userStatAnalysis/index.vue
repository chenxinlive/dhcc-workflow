...
export default {
  ...
  methods:{
    async initData(){
        const res=await getMyStartListPage(this.queryParams);
      if(res.code == '200'){
        this.total=res.data.total;
        this.list=res.data.list;
        this.processManages=res.data.processManages;
        this.refuseList=res.data.refuseList;
        this.pieData=res.data.pieData;
      }
    },
    handleView(row){
      getManageParams(row.business_key_,row.proc_inst_id_).then((res) => {
        if(res.code == '200'){
          this.form_url=res.data.form_url;
          this.form_type=res.data.form_type;
        }
        else{
          this.form_url='';
          this.form_type='';
        ...
        if(res.data.map_json==""||res.data.map_json==undefined){
          this.processKey="";
          this.processName="";
          this.mapConfig={};
          this.mapList=[];
          ...
          const mapJson = JSON.parse(res.data.map_json);
          let dj={};
          if(res.data.data_json!=""&&res.data.data_json!=null&&res.data.data_json!=undefined){
            this.dataJson=JSON.parse(res.data.data_json);
            dj=JSON.parse(res.data.data_json);
          }
          this.processKey="";
          this.processName="";
