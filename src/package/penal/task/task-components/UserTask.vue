...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        'getTaskRules':'taskRule/getTaskRules',
        'getStartRules':'taskRule/getStartRules'
      })
    },
    methods: {
      async taskLoad() {
        ...
        if (res.code == '200') {
          this.assigneeRoles = res.data.rolesList;
         // this.deptList=res.result.departList;
          this.assigneePosition = res.data.positionList;
          this.groupList=res.data.groupEntityList;
        }
      },
      //重置参数
      resetTaskForm() {
      ...
      loadAssigneeUserList(key) {
        getAssigneeUserWithTaskId({ usertaskid: this.id, ruleName: key }).then((res) => {
          if (res.code == '200') {
            if (key == 'byRole') {
              var roleIntSet = [];
              for (let roleKey in res.data.assigneeUserEntityList) {
                if(res.data.assigneeUserEntityList[roleKey].username==""||
                  res.data.assigneeUserEntityList[roleKey].username==undefined){
                  roleIntSet.push(res.data.assigneeUserEntityList[roleKey].id);
                }
                else{
                  roleIntSet.push(res.data.assigneeUserEntityList[roleKey].username);
                }
              }
              this.userTaskForm.assigneeRoles = roleIntSet;
            } else if (key == 'byPosition') {
              var postIntSet = [];
              for (let ptKey in res.data.assigneeUserEntityList) {
                if(res.data.assigneeUserEntityList[ptKey].username==""||
                  res.data.assigneeUserEntityList[ptKey].username==undefined){
                  postIntSet.push(res.data.assigneeUserEntityList[ptKey].id);
                }
                else{
                  postIntSet.push(res.data.assigneeUserEntityList[ptKey].username);
                }
              }
              ...
              for (let ptKey in res.data.assigneeUserEntityList) {
                if(res.data.assigneeUserEntityList[ptKey].username==""||
                  res.data.assigneeUserEntityList[ptKey].username==undefined){
                  groupIntSet.push(res.data.assigneeUserEntityList[ptKey].id);
                }
                else{
                  groupIntSet.push(res.data.assigneeUserEntityList[ptKey].username);
                }
              }
              this.userTaskForm.assigneeGroup = groupIntSet;
            }else {
              this.assigneeUserList = res.data.assigneeUserEntityList;
            }
          }
        });
      }, //移除节点处理人
