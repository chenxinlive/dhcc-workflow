      ...
      if (this.bpmnElementSourceRef && this.bpmnElementSourceRef.default && this.bpmnElementSourceRef.default.id === this.bpmnElement.id) {
        ...
        getFlowCond(this.bpmnElement.id).then((res)=>{
          if(res.code=="200"&&res.data!=undefined){
            this.expressionType.type=res.data.expression_type;
            if(res.data.expression_type=="sql")
              this.expressionType.sqlBody=res.data.expression_body;
            if(res.data.expression_type=="webApi")
              this.expressionType.webApiBody=res.data.expression_body;
          }
        })
      }
    },
